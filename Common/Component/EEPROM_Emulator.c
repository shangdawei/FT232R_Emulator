...
typedef struct
{
  const EEPROM_Configuration_TypeDef * Configuration;
  const Driver_EEPROM_Emulator * Driver;
...
#if ( EEPROM_EMULATOR_CACHE_ENABLED > 0 )
...
// EEPROM_SUCCESS or EEPROM_FAULT < Erase or Program Failed >
  return RetValue;
}

static void EEPROM_CalculateParameter( void )
{
  const EEPROM_Configuration_TypeDef * Configuration = Context.Configuration;
...
// (2) Transfer Old Page to New Page
...
#if ( EEPROM_EMULATOR_CACHE_ENABLED > 0 )
  uint16_t * WordCache = (uint16_t *) Context.Cache;
  for ( uint32_t WordIndex = 0; WordIndex < Context.EepromWordCount; WordIndex++ )
  {
    if ( WordIndex == ( Addr >> 1 ) )
  ...
  for ( uint32_t WordIndex = 0; WordIndex < Context.EepromWordCount; WordIndex++ )
  {
    RetValue = EEPROM_ReadWord( WordIndex << 1, &Word );
    if ( EEPROM_SUCCESS != RetValue )
    return RetValue;

    RetValue = EEPROM_WritePair( WordIndex << 1, Word );
    if ( EEPROM_SUCCESS != RetValue )         // this page is breakdown
    return RetValue;
  }
#endif
  ...
  const EEPROM_Configuration_TypeDef * EEPROM_Configuration, uint8_t * EEPROM_Cache )
{
  ...
  Context.Driver = EEPROM_Emulator_Driver;
  Context.Configuration = EEPROM_Configuration;
  ...
  do
  {
    for ( WordAddr = 0; WordAddr < WordCount; WordAddr++ )
    {
      ...
      if ( EEPROM_SUCCESS != RetValue )       // Write Failed, Try Next Page
      {
        ...
        RetValue = EEPROM_FAULT;        // Write Failed, Try Next Page
        break;
      }
    }
...
#else
  uint16_t WordData;
  RetValue = EEPROM_ReadWord( ( Addr >> 1 ) << 1, &WordData );
  if ( EEPROM_SUCCESS == RetValue )   // Gave Byte Addressed
  return RetValue;
...
#else
// Read from PageEnd downto PageStart
  /* Get the Page start and Address */
  uint32_t ValueToRead;
  uint32_t LastAddress = EEPROM_PageLastAddress( Context.PageToRead );
  uint32_t BaseAddress = EEPROM_PageBaseAddress( Context.PageToRead );
  if ( Context.PageToRead == Context.PageToWrite )  // No Transfer pending
  {
    if ( Context.AddrToWrite )
    LastAddress = Context.AddrToWrite - 4;
  }

  while ( LastAddress > BaseAddress )
  {
    /* Verify if Address contents are 0xFFFFFFFF */
    RetValue = Context.Driver->FlashRead( BaseAddress, &ValueToRead );
    if ( EEPROM_SUCCESS != RetValue )
    return RetValue;

    if ( Addr == ( ValueToRead >> 16 ) )  // Found
    {
      *Data = ValueToRead;
      return EEPROM_SUCCESS;
    }

    LastAddress -= 4;     // Next address location
  }
...
int32_t EEPROM_ReadDWord( uint32_t Addr, uint32_t * Data )
{
  ...
  RetValue = EEPROM_ReadWord( Addr, WordData );
  if ( EEPROM_SUCCESS == RetValue )
  return RetValue;
...
int32_t EEPROM_ReadBytes( uint32_t Addr, uint32_t Size, uint8_t * Data )
{
  int32_t RetValue;
  uint16_t WordData;
  if ( Addr & 1 )
  {
    RetValue = EEPROM_ReadByte( Addr, Data );
    if ( EEPROM_SUCCESS != RetValue )
      return RetValue;
...
int32_t EEPROM_WriteBytes( uint32_t Addr, uint32_t Size, uint8_t * Data )
{
  int32_t RetValue;
  uint16_t WordData;
  if ( Addr & 1 )
  {
    RetValue = EEPROM_WriteByte( Addr, Data[ 0 ] );
    if ( EEPROM_SUCCESS != RetValue )
      return RetValue;
    Size--;
    Addr++;
  ...
  while ( Size > 2 )
  {
    WordData = Data[ 0 ] | ( Data[ 1 ] << 8 );
    RetValue = EEPROM_WriteWord( Addr, WordData );
    if ( EEPROM_SUCCESS != RetValue )
      return RetValue;
    Size -= 2;
    Addr += 2;
...
static const uint8_t EepromFormatData[ 128 ] = {
...
// STM32F103VET6
#define EE_FLASH_PAGE_SIZE  ( 4096 )

static const EEPROM_Configuration_TypeDef EEPROM_Configuration = {
  0x08007000, 4, EE_FLASH_PAGE_SIZE, 128 };
static uint8_t EEPROM_Cache[ 128 ];

void EEPROM_Demo( void )
{
  if ( EEPROM_VIRGIN == EEPROM_Init( &EEPROM_Emulator_Driver, &EEPROM_Configuration, EEPROM_Cache ) )
    EEPROM_Format( (uint8_t *) EepromFormatData, 128 );
