...
/*****  End of configuration settings *******************************/
...
#elif defined (__ARM7M__) && (__CORE__ == __ARM7M__)     // Cortex-M3
  //
  // Use IAR Intrinsic functions.
...
#elif defined (__ARM7EM__) && (__CORE__ == __ARM7EM__)   // Cortex-M4/M4F/M7
  //
  // Use IAR Intrinsic functions.
  ...
  // are not affected, calls to __disable_interrupt() and __enable_interrupt() may be
  // removed if using any other CPU than Cortex-M7 r0p1.
  // The NOPs should flush the pipeline to ensure that interrupts
  // are disabled before the next instruction is executed.
  //
  #define OS_DI() {                   \
...
*       Implemented here, using IAR intrinsics
*/
#if defined (__ARM6M__) && (__CORE__ == __ARM6M__)
  #define OS_INT_PRIO_PRESERVE(p)  (*(p) = (OS_U32) __get_PRIMASK())  // Cortex-M0
  #define OS_INT_PRIO_RESTORE(p)   __set_PRIMASK(*(p))
#else
  #define OS_INT_PRIO_PRESERVE(p)  (*(p) = (OS_U32) __get_BASEPRI())  // Cortex-M3/M4/M4F/M7
  #define OS_INT_PRIO_RESTORE(p)   __set_BASEPRI(*(p));
#endif
...
#define OS_VERSION_GENERIC (42200u)       /*  Generic version 4.22 */
...
#define OS_TRACE_ID_IFE_NEXT         (4001u)
  ...
  #define TRACE_ON_TASK_STOP_READY(TaskId, Para1)                     if (OS_Global.pTrace != NULL) {OS_Global.pTrace->pfRecordTaskStopReady((TaskId), (Para1)); }
...
#define OS_TimeDex       OS_Global.TimeDex
  ...
  */
    ...
    OS_EXTERN   OS_SADDR   OS_RSEMA                *          OS_pRSema;    /*lint !e9075 MISRA C:2012 Rule 8.4, required  */ /*lint !e9075 MISRA C:2012 Rule 8.4, required  */
    OS_EXTERN   OS_SADDR   OS_Q                    *          OS_pQHead;    /*lint !e9075 MISRA C:2012 Rule 8.4, required  */ /*lint !e9075 MISRA C:2012 Rule 8.4, required  */
    #if (OS_LINK_MAILBOXES != 0)
      OS_EXTERN OS_SADDR   OS_MAILBOX              *          OS_pMailbox;  /*lint !e9075 MISRA C:2012 Rule 8.4, required  */ /*lint !e9075 MISRA C:2012 Rule 8.4, required  */
    #endif
    #if (OS_DEBUG != 0)
      OS_EXTERN OS_SADDR   OS_CSEMA                *          OS_pCSema;    /*lint !e9075 MISRA C:2012 Rule 8.4, required  */
    #endif
    OS_EXTERN   OS_SADDR   OS_MEMF                 *          OS_pMEMF;     /* linked list of all fixed size memory pools */ /*lint !e9075 MISRA C:2012 Rule 8.4, required  */ /*lint !e9075 MISRA C:2012 Rule 8.4, required  */
    OS_EXTERN   OS_SADDR   OS_TICK_HOOK            *          OS_pTickHook; /* linked list of all tick hook functions */ /*lint !e9075 MISRA C:2012 Rule 8.4, required  */ /*lint !e9075 MISRA C:2012 Rule 8.4, required  */
    #if (OS_SUPPORT_PTLS != 0)
      OS_EXTERN OS_SADDR   OS_TLS_PTR                         OS_pTLS;      /* Global pointer to thread local storage */
    #endif
    #if (OS_SUPPORT_SAVE_RESTORE_HOOK != 0)
      OS_EXTERN OS_SADDR   OS_EXTEND_TASK_CONTEXT  *          OS_pDefaultTaskContextExtension;  /*lint !e9075 MISRA C:2012 Rule 8.4, required  */ /*lint !e9075 MISRA C:2012 Rule 8.4, required  */
    #endif
  #else
...
OS_TASK* OS_TaskIndex2Ptr(int TaskIndex) OS_SECTION_ATTRIBUTE;  /* OS_TaskIndex2Ptr.c */
  ...
  void   OS_STAT_NotifyReadyStart   (OS_TASK *pTask)       OS_SECTION_ATTRIBUTE; /* OSStat.c   */
