...
/*
 * |<---------------- USB_PacketMemory : 768 Bytes ---------------------->|<---- CAN Memory ---->|
 * |BufferTable : EPx_TxBufer : EPx_RxBufer : EP0_TxBuffer : EP0_RxBuffer |<---- 256 Bytes ----->|
 * |                                                                      |<---- pma_free_offset : 0x300
 */
...
void DCD_Init( USB_CORE_HANDLE *pdev )
{
  ...
  pdev->dev.pma_free_offset = 0x300;
 ...
 * @param  pmaadress: EP address in The PMA:
 *                   In case of single buffer endpoint this parameter is a 16-bit value providing
 *                   the address in PMA allocated to endpoint.
 *
 *                   In case of double buffer endpoint this parameter is a 32-bit value providing the
...
uint32_t DCD_EP_Open( USB_CORE_HANDLE *pdev, uint8_t ep_addr, uint16_t ep_mps, uint8_t ep_type )
{
  ...
  if ( ep->pmaadress == 0 )
  {
    pdev->dev.pma_free_offset -= ep_mps;
    DCD_PMA_Config( pdev, ep_addr, USB_SNG_BUF, pdev->dev.pma_free_offset );
  }
